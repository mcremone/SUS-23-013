...
\newcommand{\mettrig}{\ensuremath{E_{\text{T, trig}}^{\text{miss}}}}
\newcommand{\mhttrig}{\ensuremath{H_{\text{T, trig}}^{\text{miss}}}}
\newcommand{\brhinv}{\ensuremath{\mathcal{B}(\PH \to \text{inv})}}
...
\newcommand{\ptmisstrig}{\ensuremath{p_{\text{T, trig}}^{\text{miss}}}}
...
\newcommand{\Rpf}{\ensuremath{R_{\text{p}/\text{f}}}\xspace}
...
\title{Search for dark matter produced in association with a dark Higgs boson decaying into a bottom quark pair in proton-proton collisions at $\sqrt{s}=13\TeV$}
...
\begin{table}[!htbp]
    ...
    \topcaption{Expected yields from background processes in the SR. The values shown are from Monte Carlo simulation and the uncertainties are statistical-only. The expected yields for a reference signal hypothesis with m$_{Z'}$ = 1000 \GeV, m$_{h_{s}}$ = 130 \GeV, and m$_{DM}$ = 150 \GeV are also reported.}
    \label{sr_yields}
    \begin{tabular}{l c c c}
        \cmsTabSkip\cmsTabSkip
         & 2016 & 2017 & 2018 \\
         \cmsTabSkip
...
\begin{table}[!htbp]
    ...
    \topcaption{Summary of requirements that define the different analysis regions. A "\checkmark" means the requirement is enforced; an "X" means the variable is left unconstrained in that region (the cut is not applied).}
    \label{tab:selection}
    \footnotesize
