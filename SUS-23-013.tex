...
For the associated production of SM vector bosons and jets (\Zvvjets, hereafter referred to as \Zjets; \Zlljets, collectively referred to as \DYjets; and \Wlvjets, collectively referred to as \Wjets), predictions with up to two partons in the final state are obtained at leading order (LO) in QCD using \MGvATNLO~\cite{MADGRAPH} version 2.4.2 with the MLM matching scheme~\cite{Mangano:2006rw} between the jets from the calculations of the matrix elements and the parton shower.
...
\begin{table}[!htbp]
    ...
    \topcaption{Expected yields from background processes in the SR. The values shown are from MC simulation, and the uncertainties are statistical only. The expected yields for a reference signal hypothesis with m$_{Z'}$ = 1000 \GeV, $m_{\darkHiggs}$ = 130 \GeV, and m$_{DM}$ = 150 \GeV are also reported.}
    \label{sr_yields}
    ...
    \hline
    m$_{Z'}$ = 1000 \GeV, $m_{\darkHiggs}$ = 130 \GeV, m$_{DM}$ = 150 \GeV              & 2329 $\pm$ 14 & 2132 $\pm$ 13 & 2337 $\pm$ 16 \\
      \hline\hline
    \end{tabular}
\end{table}
...
\centering
    ...
    \topcaption{
       Summary of statistical and systematic uncertainties included in the analysis. The value given for each rate uncertainty is the prefit maximum value. Uncertainties in the shape of the distributions are instead labeled as such. 
    }
...
The maximum likelihood fit is performed by combining the SR and CRs as well as the data sets corresponding to the data-taking years. The \recoil-vs.-$\mSD$ distributions in SR before and after the fit (``prefit'' and ``postfit'') for all three years combined are shown in Fig.~\ref{sr}.
...
No significant signal is observed above the expected background. Exclusion limits on the signal strength $\mu$ are presented for all signal hypotheses, using all data sets and categories, and calculated with the \CLs\ criterion ~\cite{CLS1,CLS2}. An asymptotic approximation of the distribution of the profile likelihood test statistic~\cite{Cowan:2010js} is employed. 


Exclusion limits are calculated in the two-dimensional parameter space of the DM and mediator masses, \mchi and \mzp, constrained by the fact that only scenarios in which the DM particle is more massive than the \darkHiggs boson are considered. The resulting exclusion limits at 95\% confidence level (CL) on $\mu$ are shown in Figs.~\ref{fig:mhs50}--\ref{fig:mhs150} for different hypotheses of the \darkHiggs boson mass. In the plots, darker shades correspond to smaller upper limits, i.e., more stringent constraints. The solid black line represents the observed 95\% CL exclusion contour, while the dashed and dotted lines indicate the median expected exclusion and its 68\% and 95\% confidence intervals, respectively. The parameter space inside the solid black boundary is excluded at 95\% CL under the model assumptions. Values of the mediator mass up to 4.5 (2.5) \TeV are excluded for an \darkHiggs boson mass of 50 (150) \GeV. The ATLAS Collaboration recently published their results on the same final state.\cite{ATLAS:2024ypx}. They present limits for \darkHiggs boson masses in the range $30 < m_{\darkHiggs} < 150$\GeV using 13\TeV pp collision data corresponding to an integrated luminosity of 140\fbinv. They observe no excess over the expected background contributions and exclude mediator masses up to 3.4\TeV for the benchmark model with $g_{\chi} = 1.0$, $g_{q} = 0.25$, and $\theta_{\text{h}} = 0.01$. When compared to the results from the CMS experiment described in this paper for an \mchi hypothesis of 200\GeV, CMS excludes larger values for \mzp for all the \darkHiggs boson mass hypotheses considered in this search (50 to 150\GeV). 
...
\section{Summary}
A search for dark matter produced in association with a dark Higgs boson in events with a resonant \PQb quark-antiquark pair and large missing transverse momentum has been presented. Dark Higgs boson mass hypotheses between 50 and 150\GeV are tested with this search. Only scenarios in which the dark matter particle is more massive than the dark Higgs boson are considered. A data set of proton-proton collisions at a center-of-mass energy of 13\TeV, corresponding to an integrated luminosity of 138\fbinv is analyzed. A joint maximum likelihood fit spanning a set of signal region and control regions is used to constrain the standard model background contributions to the data and to extract a possible signal. 

The result is interpreted in terms of exclusion limits at 95\% confidence level on the parameters of the model. Values of the mediator mass up to 4.5 (2.5) \TeV are excluded for a dark Higgs boson of 50 (150) \GeV, assuming couplings of $\gq=0.25$ between the mediator and quarks, and $g_{\chi}$=1.0 between the mediator and the DM particles. 
